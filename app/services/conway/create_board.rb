class Conway::CreateBoard
  class ValidationError < StandardError; end

  # state input sample: [[2, 1], [2, 2], [2, 3]]
  ...
  def create_board
    self.board = Board.create!(round: 0)
  end
  ...
  def validate_input
    ...
    # each initial state cell coordinate needs to be within the board dimensions
    state.each do |cell|
